...
describe('bootstrap5nt:version (default view)', () => {
  ...
  test('live mode → renders div with "bootstrap5-js-rendering" text', () => {
    ...
    expect(div?.textContent).toContain('bootstrap5-js-rendering');
  });
  ...
  test('edit mode → still shows "bootstrap5-js-rendering" text', () => {
    const ctx = makeCtx({}, { _editMode: true });
    _setContext(ctx);
    const fn = getVersionFn();
    const { container } = renderFn(fn, {}, ctx);
    expect(container.textContent).toContain('bootstrap5-js-rendering');
  });
});
